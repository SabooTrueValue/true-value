...
const allApiData = async (req, res) => {
    try{
        ...
        // const buyVehicleData = await buyVehicleModel.find();
        ...
        const contactusData = await contactusModel.find().count();
         res.status(200).send({soldVehicleData, availableVehicleData, financeData, homequeryData, popupData, sellData, userData, vehicleData, contactusData});
    }
    catch (error) {
        console.log(error);
    }
